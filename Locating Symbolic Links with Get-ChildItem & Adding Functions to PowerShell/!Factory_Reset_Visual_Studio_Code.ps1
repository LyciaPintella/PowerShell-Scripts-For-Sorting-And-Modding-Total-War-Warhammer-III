# ~ VS Code Factory Reset Script # 1
$paths = @(
  "$env:APPDATA\Code",
...
if (-Not (Read-Host "Type 'YES' to proceed") -eq 'YES') {
  Write-Host "Aborted by user"; exit
}
foreach ($p in $paths) {
  if (Test-Path $p) {
    Rename-Item $p "$p.backup" -ErrorAction SilentlyContinue
  }
}
Write-Host "Reset complete. Restart VS Code."
...
# ~ VS Code Factory Reset Script # 2
